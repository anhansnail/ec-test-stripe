    ...
    public function edit(Request $request, $id = null, RouterInterface $router, CacheUtil $cacheUtil)
    {
        ...
        if ('POST' === $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                \Stripe\Stripe::setApiKey($this->BaseInfo->getStripeSecretKey());
...
//                    $stripe = \Stripe::setApiKey('sk_test_ulFMDa6ztpmBFmvzl42TMeis00EKxiEHH6');
//                    $sp = \Stripe\Product::create([
//                        'name' => $Product->getName(),
//                    ]);
                    $sp = $stripe->products->create([
                        'name' => $Product->getName(),
                    ]);
...
//                    ];
                    $Product->setProductStripe($sp->id);
                    $Product->setPriceStripe($price->id);
                    $this->entityManager->persist($Product);
                    $this->entityManager->flush();
                }
